...
Side note: You can specify as many different lights as you want to turn on and assign different behavior to each - just make sure to name them all the same thing.  Ex: I can 100 lights named "BlueLight", 200 named "PurpleLight", etc. and assign different lighting settings to each group of lights.

Now, one key problem exists in this? **What if I have a multi-instance light - won't that create an error with regards to randomization**  Yes!  If you're not following what this error is, imagine a lantern model.  The lantern model has a bunch of wooden structure parts, a glass part, and a PointLight within the glass part.  I can set the PointLight to have a 50% chance of turning on, but then my glass part would look unlit.  Or I could set the glass part to have a 50% chance of turning on (again, abusing that neon effect and turning it maybe a yellow color) - but then no light would radiate out of it.  D:  And, if I set them both to turn on (with 50% probability of turning on), there's only a 25% of them both turning on at the same time, and more than likely only one of them will turn on.  
...
I tried to make this Module as intuitive as possible in my quest to make it both strong, bug resistant (if you find any let me know), and user-friendly.  Here are some cool smart features in the Module:
...
``` 
which will tween your current lighting settings to those of the weather.  This is useful for creating dark storms, gray/white snow storms, etc.  Currently the settings that can be adjusted in weather settings are FogEnd, FogColor, Size of BlurEffects, Intesity of SunRays, Brightness, Ambient, OutdoorAmbient, ShadowSoftness, WaterReflectance, WaterWaveSize, and WaterWaveSpeed.  Just like Lighting Settings, this list can definitely be expanded - I just thought these were the most relevant to start with.
